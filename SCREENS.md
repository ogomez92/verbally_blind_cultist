...
| Table: verb window | `SituationWindow`, `ThresholdSphere`, `NotesSphere`, `OutputSphere` | Enter on verb, or game opens it | TabletopNavigator Window | story text (pages), slots, Start, timer, stored cards, deck draws, results, Collect all, aspect totals | slot requirements (details), aspect meanings | verified (unstarted, running, complete, face-down results) |
